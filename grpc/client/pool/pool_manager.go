...
func (m *poolManager) get(opts ...grpc.DialOption) (*ClientConn, error) {
	...
	if found {
		tracer.AddTrace("got connection at try-find-one: ", m.addr)
		return conn, nil
	}
	...
	if found {
		...
		m.tickets.release()
		tracer.AddTrace("got connection at try-find-one after ticket: ", m.addr)
		return conn, nil
	}

	tracer.AddTrace("got connection at creating new one: ", m.addr)
...
func (m *poolManager) tryFindOne() (*ClientConn, bool) {
	...
	for idx, conn := range m.indexes {
		...
		tracer.AddTrace("connection: ", unsafe.Pointer(conn), " with ref-count: ", conn.refCount)
		return conn, true
	}
...
func (m *poolManager) backoff(cc *grpc.ClientConn) time.Time {
	...
	tracer.AddTrace(fmt.Sprintf("address: %s  with new backoff: %v", m.addr, p))
...
func (m *poolManager) put(conn *ClientConn, err error) {
	...
	if conn.newCreated {
		...
		tracer.AddTrace("new created release ticket: ", m.addr, err)
		m.tickets.release()
	}
	...
	if err != nil {
		conn.closable = true
		tracer.AddTrace(conn, "@", m.addr, " got error: ", err)
	}
	...
	if !inPool {
		if len(m.data) >= m.size {
			conn.closable = true
			tracer.AddTrace("pool is full.", m.addr)
		}
	}
	...
	if conn.refCount <= 0 {
		go func() {
			...
			if err := conn.ClientConn.Close(); err != nil {
				tracer.AddTrace("close client connection:", conn, " got error: ", err)
			}
		}()
	}
}
...
func (m *poolManager) cleanup() {
	...
	for conn := range m.data {
		if conn.refCount > 0 {
			tracer.AddTrace("haven't been closed: ", unsafe.Pointer(conn), "@", m.addr)
		}
...
func newTicker(size int) ticket {
	...
	tracer.AddTrace("create a new ticket. size is: ", size)
	return tks
}

func (t ticket) get() {
	<-t
	tracer.AddTrace("size after get: ", t.size())
}

func (t ticket) release() {
	t <- struct{}{}
	tracer.AddTrace("size after release: ", t.size())
}
